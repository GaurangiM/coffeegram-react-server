...
router.post("/postreview", authMiddleware, async(req, res)=> {
  const { cafeId, review, rating, image, userId } = req.body;
  try {
    ...
    res.status(201).send({message:"Review posted", userCafe});
  }catch(e) {
    console.log(e.message)
    next(e)
  }
...
router.post("/postnewcafe", authMiddleware, async(req, res, next)=> {
  ...
  try {
    ...
    res.status(201).send({message:"New address posted", newAddress, newCafe});
  }catch(e) {
    console.log(e.message)
    next(e)
  }
